...
import { ArrowLeft, Settings, AlertTriangle, Info, Route, Wrench, CheckCircle2 } from 'lucide-react';
...
const NotificationsScreen: React.FC<NotificationsScreenProps> = ({ onNavigate }) => {
  const notifications = INDIA_NOTIFICATIONS;
    ...
    <div className="h-full bg-[#F5F7FA] flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 bg-white/80 backdrop-blur-md px-5 py-4 pt-10 flex items-center justify-between border-b border-white/40 shadow-sm">
        ...
        <IconButton variant="secondary" size="md">
          <Settings size={18} className="text-neutral-dark" />
        </IconButton>
      </div>
        ...
        <div className="flex space-x-2 overflow-x-auto no-scrollbar px-3">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                {tab} {tab === 'All' && <span className="ml-1 opacity-60">8</span>}
              </button>
            )
          })}
        </div>
      ...
      <div className="flex-1 overflow-y-auto pt-36 pb-24 px-4 space-y-4 no-scrollbar">
        {notifications.map((notification) => {
          ...
          return (
            <Card key={notification.id} className="!p-4 border-none shadow-sm relative overflow-hidden group">
              <div className="flex space-x-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-neutral-medium text-[13px] mb-3 leading-snug">
